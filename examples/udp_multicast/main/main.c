/**
 * UDP multicast receiver example — ported from WIZnet-PICO-C
 * examples/udp_multicast/udp_multicast_receiver.
 *
 * Joins multicast group 224.0.0.5:30000 and prints received datagrams
 * (uses the official ioLibrary Application/multicast helper).
 *
...
static void multicast_task(void *arg)
{
    (void)arg;

    wizchip_port_initialize();

    printf("multicast group %d.%d.%d.%d:%d\n",
           g_multicast_ip[0], g_multicast_ip[1], g_multicast_ip[2], g_multicast_ip[3],
           g_multicast_port);

    /* Infinite loop */
    while (1) {
        // Multicast receive test
        multicast_recv(SOCKET_ID, g_ethernet_buf, g_multicast_ip, g_multicast_port);
...
void app_main(void)
{
    ...
    xTaskCreate(multicast_task, "multicast_task", 8192, NULL, 5, NULL);
}
