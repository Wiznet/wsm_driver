/**
 * SNTP example — ported from WIZnet-PICO-C examples/sntp.
 *
 * Queries time.google.com (216.239.35.0) over SNTP and prints the current
 * date/time. millis() uses esp_timer instead of the Pico repeating timer.
 *
...
static void sntp_task(void *arg)
{
    ...
    /* Infinite loop */
    while (1) {
        vTaskDelay(pdMS_TO_TICKS(1000)); // nothing to do
    }
}
