 ...
 *
 * Wi-Fi socket vtable for the backend-neutral SNTP client (sntp_client.c). The
 * Wi-Fi and Ethernet clients are started identically via sntp_client_start();
 ...
 * reach the REAL software LwIP stack that the Wi-Fi netif is attached to.
 *
 ...
 * ops). The two clients bind different local ports (see net_config.h) to avoid
 * a clash on that shared stack.
 */
#include "lwip/sockets.h"

#include "sntp_client.h"   /* sntp_ops_t + wifi_sntp_ops declaration */
...
const sntp_ops_t wifi_sntp_ops = {
