 ...
 *
 * This is the ONE place aware of the W5500 --wrap. In ESP_WIZ_TOE_SOCKET_WRAP=1
 * the plain lwip_* symbols are redirected to the W5500 hardware sockets, so Wi-Fi
 * must bind to the linker's __real_lwip_* (the un-wrapped originals) to reach the
 ...
 *
 * In ESP_WIZ_TOE_SOCKET_WRAP=0 there is no --wrap; Wi-Fi and Ethernet share one LwIP stack,
...
#include "loopback.h"   /* loopback_ops_t + wifi_loopback_ops declaration */
 ...
 * ESP_WIZ_TOE_SOCKET_WRAP PUBLIC). Calling __real_* bypasses the W5500 redirect
